...
const assignTasks = (members, tasks) => {
    ...
    let filteredTasks = tasks.filter(task => task.assignees === undefined).sort((a, b) => a.difficulty > b.difficulty);
    ...
    newTasks.forEach(task => {
        ...
        for (let i = 0; i < task.difficulty; i++) {
            ...
            task.assignees.push(curMember.name); // since we don't need to store the assignees' scores at the time of assignment, right?
        }
    ...
    return {tasks: [...tasks.filter(task => task.assignees !== undefined), ...newTasks], mems: updatedMembers}
}
